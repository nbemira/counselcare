...
<head>
    ...
    <style>
        .dropdown-menu {
            min-width: 8rem; /* Adjust this value to decrease the width */
        }
    </style>
</head>
    ...
    <nav class="bg-blue-100 text-gray-700 border-b border-gray-200 shadow-lg">
            ...
            <div class="flex justify-between h-20"> <!-- Increased height to accommodate larger logo -->
                <div class="flex">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        ...
                        <x-nav-link :href="route('student.assessment')" :active="request()->routeIs('student.assessment')">
                ...
                <div class="hidden sm:flex sm:items-center sm:ml-6 relative">
                    <div class="relative">
                        ...
                        <div id="dropdown-menu" class="hidden dropdown-menu absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-md shadow-lg py-1 z-50">
                            <a href="{{ route('student.profile.update-password-form') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 flex items-center">
                            ...
                            <form method="POST" action="{{ route('student.logout') }}">
                                @csrf
                                <a href="{{ route('student.logout') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 flex items-center"
        ...
        <div id="responsive-menu" class="hidden sm:hidden bg-blue-300">
            <div class="pt-2 pb-3 space-y-1">
                ...
                <x-responsive-nav-link :href="route('student.assessment')" :active="request()->routeIs('student.assessment')">
                    {{ __('Mental Health Assessment') }}
                </x-responsive-nav-link>
            </div>
            ...
            <div class="pt-4 pb-1 border-t border-gray-200">
                <div class="px-4">
                    <div class="font-medium text-base text-white">{{ Auth::guard('student')->user()->name }}</div>
                    <div class="font-medium text-sm text-gray-500">{{ Auth::guard('student')->user()->email }}</div>
                </div>

                <div class="mt-3 space-y-1">
                    <x-responsive-nav-link :href="route('student.profile.update-password-form')">
                        {{ __('Update Password') }}
                    </x-responsive-nav-link>
