    ...
    <nav class="bg-blue-100 text-gray-700 border-b border-gray-200 shadow-lg">
            ...
            <div class="flex justify-between h-20">
                <div class="flex">
                    ...
                    <div class="shrink-0 flex items-center">
                        <a href="{{ route('counsellor.assessment') }}">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        <x-nav-link :href="route('counsellor.assessment')" :active="request()->routeIs('counsellor.assessment')">
                            {{ __('Mental Health Assessment') }}
                        </x-nav-link>
                        <x-nav-link :href="route('counsellor.dashboard')" :active="request()->routeIs('counsellor.dashboard')">
                            {{ __('Dashboard') }}
                        </x-nav-link>
                        <x-nav-link :href="route('counsellor.case-management')" :active="request()->routeIs('counsellor.case-management')">
                            {{ __('Case Management') }}
                        </x-nav-link>
                    </div>
                </div>
                    ...
                    <div class="relative">
                        <button onclick="toggleDropdown()" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-700 focus:outline-none transition ease-in-out duration-150 transform hover:scale-105">
                            ...
                            <div class="ml-1">
                                <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 011.414 1.414l-4 4a1 1 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                </svg>
                            </div>
                        </button>
                        ...
                        <div id="dropdown-menu" class="hidden absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-md shadow-lg py-1 z-50">
                            <a href="{{ route('profile.update-password-form') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2 icon icon-tabler icon-tabler-password-user">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <a href="{{ route('logout') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 flex items-center"
                                   onclick="event.preventDefault(); this.closest('form').submit();">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2 icon icon-tabler icon-tabler-logout"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M14 8v-2a2 2 0 0 0 -2 -2h-7a2 2 0 0 0 -2 2v12a2 2 0 0 0 2 2h7a2 2 0 0 0 2 -2v-2" /><path d="M9 12h12l-3 -3" /><path d="M18 15l3 -3" /></svg>
                                    {{ __('Log Out') }}
                                </a>
                            </form>
                        </div>
        ...
        <div id="responsive-menu" class="hidden sm:hidden bg-blue-300">
            <div class="pt-2 pb-3 space-y-1">
                <x-responsive-nav-link :href="route('counsellor.assessment')" :active="request()->routeIs('counsellor.assessment')">
                    {{ __('Mental Health Assessment') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('counsellor.dashboard')" :active="request()->routeIs('counsellor.dashboard')">
                    {{ __('Dashboard') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('counsellor.case-management')" :active="request()->routeIs('counsellor.case-management')">
                    {{ __('Case Management') }}
                </x-responsive-nav-link>
            </div>
